...
describe('regrowFor', () => {
  it('gives nothing back without a streak', () => {
    expect(regrowFor(0)).toBe(0)
    expect(regrowFor(-3)).toBe(0)
  })

  it('matches the base reward for a lone perfect drop', () => {
    expect(regrowFor(1)).toBeCloseTo(PERFECT_REGROW)
  })

  it('rewards a longer streak with more size', () => {
    expect(regrowFor(2)).toBeGreaterThan(regrowFor(1))
    expect(regrowFor(5)).toBeGreaterThan(regrowFor(2))
  })
  ...
  it('increases monotonically and never exceeds the cap', () => {
    ...
    for (let c = 1; c <= 60; c++) {
      ...
      expect(r).toBeLessThanOrEqual(PERFECT_REGROW_MAX + 1e-9)
      prev = r
    }
    expect(regrowFor(9999)).toBeCloseTo(PERFECT_REGROW_MAX)
  })
})

describe('resolveDrop with a custom regrow', () => {
  it('hands back exactly the amount asked for', () => {
    const narrow: Slab = { x: 0, z: 0, w: 1.5, d: BASE_SIZE, y: 0 }
    const r = resolveDrop(narrow, 0, 'x', 1, 0.2)
    if (r.kind !== 'perfect') throw new Error('expected perfect')
    expect(r.slab.w).toBeCloseTo(1.7)
  })

  it('still refuses to grow past the original base size', () => {
    const wide: Slab = { x: 0, z: 0, w: BASE_SIZE - 0.05, d: BASE_SIZE, y: 0 }
    const r = resolveDrop(wide, 0, 'x', 1, 5)
    if (r.kind !== 'perfect') throw new Error('expected perfect')
    expect(r.slab.w).toBe(BASE_SIZE)
  })

  it('a streak recovers a squeezed tower faster than single perfects', () => {
    let solo = 1.0
    let streak = 1.0
    for (let i = 1; i <= 6; i++) {
      const a = resolveDrop({ x: 0, z: 0, w: solo, d: 3, y: 0 }, 0, 'x', 1, regrowFor(1))
      if (a.kind === 'perfect') solo = a.slab.w
      const b = resolveDrop({ x: 0, z: 0, w: streak, d: 3, y: 0 }, 0, 'x', 1, regrowFor(i))
